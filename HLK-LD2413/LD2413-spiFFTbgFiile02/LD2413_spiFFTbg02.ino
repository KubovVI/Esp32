...
#include "FS.h"
#include <LittleFS.h>
#define FileName "/BGfile.bin" 

bool LogEnable, LogSPIenable, LogPreFFTenable, LogFFTenable, LogBGenable;
bool winFFTenable, removeBGenable, saveAiqFile;
...
void writeDataFile(const char *path, float *Re,float *Im,int N) {
  ...
  if (!file) { Serial.print(path); Serial.println(" - Error File Write!"); return; }
  n1=file.write((uint8_t*) Re, byteSize);
  n2=file.write((uint8_t*) Im, byteSize);
  Serial.print(path); Serial.print("<- "); 
  Serial.print(n1); Serial.print("+");Serial.print(n2); Serial.println(" bytes writed");
  file.close();
}//writeDataFile

void readDataFile(const char *path, float *Re,float *Im,int N) {
  int n1,n2;
  int byteSize=N*4;
  File file = LittleFS.open(path);
  if (!file) { Serial.print(path); Serial.println(" - Error File Read!"); return; }
  n1=file.read((uint8_t*) Re, byteSize);
  n2=file.read((uint8_t*) Im, byteSize);
  Serial.print(path); Serial.print("-> "); 
  Serial.print(n1); Serial.print("+");Serial.print(n2); Serial.println(" bytes readed");
  file.close();
}//writeDataFile

void handleBG(){
  if (LogEnable) Serial.println("handleBG");
  String s="#\t I\t Q\t"+String(numbBGavrg)+"avrg\n";
  server.setContentLength(CONTENT_LENGTH_UNKNOWN);
  server.send(200,"text/plain",s);
  for (int n=0; n<FFTsize; n++){ s=String(n)+"\t"; 
    s+=(String(bgRe[n],0)+"\t"+String(bgIm[n],0)+"\n");
    server.sendContent(s);
  }//for n
  server.sendContent(""); //close
}//handleBG 
...
void decodeWS(String s){
  String s1, mask;
  int p;
  mask="#w="; p=s.indexOf(mask);
  if (p>=0){ String s1=s.substring(mask.length());
    if (s1.charAt(0)=='0') winFFTenable=0; else winFFTenable=1;
  }//if  
  mask="#bg="; p=s.indexOf(mask);
  if (p>=0){ s1=s.substring(mask.length());
    if (s1.charAt(0)=='0') removeBGenable=0; else removeBGenable=1;
  }//if  
  mask="##writeBGfile"; p=s.indexOf(mask); if (p>=0) saveAiqFile=1;
  }//decodeWS
...
void setup(void) {
  ...
  LogEnable=1; LogSPIenable=0; LogPreFFTenable=0; LogFFTenable=0; LogBGenable=0;//0/1;
  winFFTenable=1; removeBGenable=1; saveAiqFile=0;
  ...
  if (!LittleFS.begin(true)) {Serial.println("Error File System!"); return;}
  readDataFile(FileName, bgRe,bgIm,FFTsize);
}//setup
...
void loop(void) {
  ...
  configStr+="; delBG:"; if (removeBGenable) configStr+="Y"; else configStr+="N";
  //preFFT(rx_buf,BUFFER_SIZE, AmpRe,AmpIm,FFTsize, winFFTenable); //set data for FFT
  getAiq(rx_buf,BUFFER_SIZE, AmpRe,AmpIm,FFTsize);
  if (saveAiqFile) { configStr="-----------"; procAiqFile(numbBGavrg); }
  if (removeBGenable) removeBg(AmpRe,AmpIm,FFTsize);
  if (winFFTenable) winAiq(AmpRe,AmpIm,FFTsize);
